 ...
 * ------------------------------------------------------------------------
 *
 * This script is adapted from an open-source project.
 * The original author and source will be credited here once identified.
 */
...
async function getDOM(link) {
	try {
		...
		if (dataJSON && dataJSON.parse && dataJSON.parse.text && dataJSON.parse.text['*']) {
			...
			console.warn(`Warning: Invalid or empty API response for page: ${link}`);
			return null;
		}
	} catch (error) {
		console.error(`Error fetching or parsing data from ${link}:`, error);
		return null;
	}
}

async function getAMC10Data(chunkDone) {
	const contestListDOM = await getDOM("https://artofproblemsolving.com/wiki/api.php?action=parse&page=AMC_10_Problems_and_Solutions&format=json");

	if (!contestListDOM) {
		console.error("Fatal: Could not fetch the main contest list. Aborting scraper.");
		return {};
	}
  ...
  for (let contestLink of contestLinks) {
    ...
    if (!contestProblemListDOM || !contestAnswerListDOM) {
      console.warn(`Skipping contest ${serializedContestTitle} due to missing problem or answer key page.`);
      continue;
    }
    ...
    for (let [problemIndex, problemLink] of problemLinks.entries()) {
	      ...
	      finalData[serializedContestTitle].problems[serializedProblemTitle] = {
	        ...
	        data: {
	          problem: "",
	          solutions: {},
	          answer: answers[problemIndex] ? answers[problemIndex].textContent : "N/A"
	        }
	      }
          ...
          if (!problemDOM) {
            console.warn(`Skipping problem ${serializedProblemTitle} due to fetch error.`);
            resolve("");
            return;
          }
