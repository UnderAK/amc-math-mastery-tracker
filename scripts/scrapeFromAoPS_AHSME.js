...
async function getAHSMEData(logProgress) {
...
async function main() {
	console.log('Starting AHSME scraper...');
	const scrapedData = await getAHSMEData(console.log);
	const outputPath = path.join(process.cwd(), 'scripts', 'ahsme.json');
	fs.writeFileSync(outputPath, JSON.stringify(scrapedData, null, 2));
	console.log(`\n✅ AHSME data scraped successfully and saved to ${outputPath}`);
}

main().catch((error) => {
	console.error('\n❌ An error occurred during the AHSME scraping process:');
	console.error(error);
	process.exit(1);
});
