...
async function getAMC8Data(logProgress) {
	const contestListDOM = await getDOM('https://artofproblemsolving.com/wiki/api.php?action=parse&page=AMC_8_Problems_and_Solutions&format=json');
	if (!contestListDOM) {
	...
	const contestLinks = contestListDOM.window.document.querySelectorAll('ul > li > a');

	for (let contestLink of contestLinks) {
		const serializedContestTitle = contestLink.textContent.replace(/\ /g, '_');

		if (isNaN(parseInt(contestLink.textContent[0]))) continue;
		...
		logProgress(`Processing Contest: ${contestLink.textContent}`);
		...
		if (!contestProblemListDOM || !contestAnswerListDOM) {
			logProgress(`  -> Skipping ${contestLink.textContent} due to missing problem or answer pages.`);
			continue;
		}
...
async function main() {
	console.log('Starting AMC 8 scraper...');
	const scrapedData = await getAMC8Data(console.log);
	const outputPath = path.join(process.cwd(), 'scripts', 'amc8.json');
	fs.writeFileSync(outputPath, JSON.stringify(scrapedData, null, 2));
	console.log(`\n✅ AMC 8 data scraped successfully and saved to ${outputPath}`);
}

main().catch((error) => {
	console.error('\n❌ An error occurred during the AMC 8 scraping process:');
	console.error(error);
	process.exit(1);
});
